...
<ul>
    ...
    <div className="elem">
     <Link to={"/Internship"}>   <p id='int' className='' onMouseEnter={showInternShips} > Internships </p></Link>
     <i onClick={hideInternShips}  id='ico' class="bi bi-caret-down-fill"></i>
       <Link to={"/Jobs"}> <p onMouseEnter={showJobs} >Jobs  </p></Link>
    ...
    }
  {
    user?(
        <>
        <button className='bt-log ml-2' id='bt' onClick={logoutFunction}>Logout <i class="bi bi-box-arrow-right"></i></button>
        </>
    ):(
        <>
...
<p onClick={loginFunction} className='flex
 items-center h-9 justify-center mt-4 text-white bg-slate-100 rounded-lg hover:bg-gray-100' >
    ...
    <h4 className='text-gray-500 cursor-pointer'>Login With Google 
    </h4>
 </p>
...
<p className='text-sm'>new to internarea? Register(<span className='text-blue-500 cursor-pointer' onClick={closeLogin}>Student</span>/<span className='text-blue-500 cursor-pointer' onClick={closeLogin}>company</span>) </p>
    ...
    {isDivVisibleForProfile&&(
        <div className="profile-dropdown h-16 rounded-sm shadow-sm">
            ...
            <Link to={"/userapplication"}>   <p id='int' className='mt-5'> View Applications </p></Link>
            <Link to={"/"}>   <p id='int' className='mt-3'> Home </p></Link>
        </div>
    )
